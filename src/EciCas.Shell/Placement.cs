...
internal sealed record Placement(double Left, double Top)
{
    ...
    /// <summary>
    /// Remembers where she rests. Left and top are passed in rather than read
    /// off the window because the window may be wider or taller than her
    /// resting size at the moment it is dragged -- it grows outward and upward
    /// to fit a long reply or transcript -- and the corner worth remembering is
    /// the one she returns to.
    /// </summary>
    public static void Save(Window window, double left, double top)
    {
        try
        {
            File.WriteAllText(Path, JsonSerializer.Serialize(new Placement(left, top)));
        }
        catch (Exception)
        {
