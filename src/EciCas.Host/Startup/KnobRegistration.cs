...
internal static class KnobRegistration
{
    public static IServiceCollection AddKnobs(this WebApplicationBuilder builder, string tier)
    {
        ...
        // constant: KnobDefaults.RecallDepth is the tier's answer to "how many rows
        // the read path hands back". Seeding it here is what makes that option mean
        // anything -- until this, every tier ran at the knob's hardcoded 5 whatever
        // it configured, and the field was read by nothing at all.
        services.AddSingleton(sp => new RuntimeKnobs
        {
            RecallDepth = sp.GetRequiredService<IOptions<KnobDefaults>>().Value.RecallDepth,
