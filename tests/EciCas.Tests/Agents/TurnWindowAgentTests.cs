...
/// shows: the turn being answered, and every idea but the newest.
/// </summary>
public class TurnWindowAgentTests
{
    ...
    private static async Task<Guid> Turn(TurnWindowAgent window, string given, string? replied, bool self = false)
    {
    ...
    public async Task ConcludedTurnsComeBackOldestFirst()
    {
        ...
        var recent = window.Recent(5, Guid.NewGuid());
    ...
    public async Task TheTurnBeingAnsweredIsNotItsOwnHistory()
    {
        ...
        Assert.Equal([("one", "first")], window.Recent(5, current));
    }
    ...
    public async Task AWindowOfZeroShowsNothing()
    {
        ...
        Assert.Empty(window.Recent(0, Guid.NewGuid()));
    }
    ...
    public async Task TheNewestIdeaIsBroughtForwardWhenTheSliceMissesIt()
    {
        ...
        var recent = window.Recent(2, Guid.NewGuid());
