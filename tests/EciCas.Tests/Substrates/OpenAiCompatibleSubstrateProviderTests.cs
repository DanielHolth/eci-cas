...
/// tier — one model behind every agent — but the absence assertions are what
...
public class OpenAiCompatibleSubstrateProviderTests
{
    ...
    private static SubstrateOptions WithAgent(SubstrateAgentEntry entry) =>
        new() { Agents = { ["Librarian"] = entry } };

    [Fact]
    public async Task WhenEntrySetsNeither_BodyCarriesNoLocalOnlyFields()
    {
        var handler = new RecordingHandler(Ok);
        var provider = Create(handler, WithAgent(new SubstrateAgentEntry { Provider = "openai" }));

        await provider.CompleteAsync("Librarian", "hello", CancellationToken.None);
    ...
    [Fact]
    public async Task WhenEntrySetsMaxTokensAndThinking_BothReachTheBody()
    {
        var handler = new RecordingHandler(Ok);
        var provider = Create(handler, WithAgent(new SubstrateAgentEntry { Provider = "local", MaxTokens = 512, Thinking = false }));
    ...
    public async Task WhenMaxConcurrentIsSet_CallsQueueInsteadOfOverlapping()
    {
        ...
        var provider = Create(handler, WithAgent(new SubstrateAgentEntry { Provider = "local" }), maxConcurrent: 2);

        await Task.WhenAll(Enumerable.Range(0, 6)
            .Select(_ => provider.CompleteAsync("Librarian", "hello", CancellationToken.None)));
    ...
    public async Task LeadingReasoningBlockIsStripped(string content, string expected)
    {
        var json = """{"choices":[{"message":{"role":"assistant","content":"""
            ...
            + """}}],"usage":null}""";
        var provider = Create(new RecordingHandler(json), WithAgent(new SubstrateAgentEntry { Provider = "local" }));

        var result = await provider.CompleteAsync("Librarian", "hello", CancellationToken.None);
